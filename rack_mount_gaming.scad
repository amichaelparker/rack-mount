// rack_mount_gaming.scad — 19" 4U open tray rack mount (GAMING VARIANT)
// ATX motherboard (flat) + Zotac RTX 5090 Solid (vertical, 67.8mm wide / 329.7mm long) + ATX PSU
// Front panel mounts a 360mm radiator (3× 120mm fans on 120mm pitch)
// Tray depth increased to 400mm vs 350mm server variant to accommodate the longer GPU.
//
...
tray_d   = 400;  // deeper than server variant (350mm) to fit 329.7mm GPU + clearance
...
gpu_l    = 329.7;  // Zotac 5090 Solid: 329.7mm length
gpu_zh   = 137.8;  // Zotac 5090 Solid: 137.8mm height (PCIe edge to top)
gpu_xw   = 67.8;   // Zotac 5090 Solid: 67.8mm wide (3.5 slots)
pcie_bx  = 110;
pcie_by  = 50;
gpu_x0   = bx0 + pcie_bx - gpu_xw/2;
gpu_y0   = by0 + pcie_by - 134.9;  // adjusted for longer GPU (was -73)

bkt_cut_w   = gpu_xw + 1;
bkt_cut_h   = 140;  // taller to match 5090 Solid height (was 121)
bkt_cut_z0  = tw;
...
tab_sw  = 10.5;
tab_sd  = 5;
tab_x   = [gpu_x0 + gpu_xw*0.2, gpu_x0 + gpu_xw*0.5, gpu_x0 + gpu_xw*0.8];  // 3 slots for wider 5090 bracket

fan_sq   = 120;
fan_msp  = 105;
// Three fans on 120mm pitch — matches a 360mm radiator (screw holes align to rad holes)
fan1_cx  = tray_w/2 - 120;  // 102mm — leftmost
fan2_cx  = tray_w/2;         // 222mm — centre
fan3_cx  = tray_w/2 + 120;  // 342mm — rightmost
fan_cz   = rack_h / 2;
...
module rear_panel_solid() {
    ...
    difference() {
        ...
        // Retainer bar screw holes through rear panel — left, centre, right (3 screws on gaming variant)
        for (bx = [bkt_cut_x0-7,
                   bkt_cut_x0 - (ret_bar_w-bkt_cut_w)/2 + ret_bar_w/2,
                   bkt_cut_x0+bkt_cut_w+7])
...
module retainer_bar_bosses() {
    // Centre boss X — bar is centred on bkt_cut: bar_x0 = bkt_cut_x0 - (ret_bar_w-bkt_cut_w)/2
    bar_x0_   = bkt_cut_x0 - (ret_bar_w - bkt_cut_w) / 2;
    centre_bx = bar_x0_ + ret_bar_w / 2;  // world X of centre screw
    difference() {
        union() {
            ...
            // Centre boss — needed for the extra middle screw on the wider gaming bar
            translate([centre_bx-5,            tray_d-tw-ret_boss_inset, ret_bar_z0-1]) cube([10, ret_boss_inset, ret_bar_h+2]);
        }
        // Clearance holes — left, centre, right
        for (bx = [bkt_cut_x0-7, centre_bx, bkt_cut_x0+bkt_cut_w+7])
...
module gpu_retainer_bar() {
    difference() {
        ...
        // Panel mounting screw holes — 3 screws: left, centre, right (wider bar needs centre)
        for (bx = [7, ret_bar_w/2, ret_bar_w-7])
        ...
        // GPU bracket screw holes — 3 screws spread across the lip to match wider 5090 bracket
        for (gx = [ret_bar_w/2 - 22.5, ret_bar_w/2, ret_bar_w/2 + 22.5])
